...
void gotMessageCallback(uint32_t from, String & msg) { // REQUIRED
  ...
  if (receipent == IDENTITY) {
    ...
    switch (message)
    {
    case VOICE3_WORD_SING:
      Serial.println("voice3: ie ie ie");
...
void greeting() {
  static String msg = "";
  sprintf(msg_cstr, "[%06d:%03d]", memberList[random(NUM_OF_MEMBERS)], VOICE3_WORD_HELLO); //"Duncan. Living in the triangularities."
  msg = String(msg_cstr);
...
void routine() {
  static String msg = "";
  sprintf(msg_cstr, "[%06d:%03d]", ID_VOICE1, VOICE1_WORD_SING);
  msg = String(msg_cstr);
  mesh.sendBroadcast(msg);
  Serial.println("to voice1");
  //
