...
void gotMessageCallback(uint32_t from, String & msg) { // REQUIRED
  ...
  if (receipent == IDENTITY) {
    ...
    switch (message)
    {
    case VOICE2_WORD_SING:
      Serial.println("voice2: oh oh oh");
...
void greeting() {
  static String msg = "";
  sprintf(msg_cstr, "[%06d:%03d]", memberList[random(NUM_OF_MEMBERS)], VOICE2_WORD_HELLO); //"Duncan. Living in the triangularities."
  msg = String(msg_cstr);
...
void routine() {
  static String msg = "";
  sprintf(msg_cstr, "[%06d:%03d]", ID_VOICE3, VOICE3_WORD_SING);
  msg = String(msg_cstr);
  mesh.sendBroadcast(msg);
  Serial.println("to voice3");
  //
