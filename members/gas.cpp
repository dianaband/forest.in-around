...
void gotMessageCallback(uint32_t from, String & msg) { // REQUIRED
  ...
  if (receipent == IDENTITY) {
    ...
    switch (message) {
    case GAS_WORD_RING_RING_RING:
      Serial.println("gas: ring ring.");
...
void greeting() {
  static String msg = "";
  sprintf(msg_cstr, "[%06d:%03d]", memberList[random(NUM_OF_MEMBERS)], GAS_WORD_HELLO); //"signal out~ take my signal~~ to every~~"
  msg = String(msg_cstr);
...
void routine() {
  ...
  if (mood == MOOD_HIGH) {
    ...
    routine_task.restartDelayed(random(5000, 10000));
