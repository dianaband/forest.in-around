...
void gotMessageCallback(uint32_t from, String & msg) { // REQUIRED
  ...
  if (receipent == IDENTITY) {
    ...
    switch (message)
    {
    case FALL2_WORD_SING:
      Serial.println("fall2: tik, tik, tik");
...
void greeting() {
  static String msg = "";
  sprintf(msg_cstr, "[%06d:%03d]", memberList[random(NUM_OF_MEMBERS)], FALL2_WORD_HELLO); //"(kuhhhkuhhhuuu)"
  msg = String(msg_cstr);
...
void routine() {
  static String msg = "";
  sprintf(msg_cstr, "[%06d:%03d]", ID_FALL1, FALL1_WORD_SING);
  msg = String(msg_cstr);
  mesh.sendBroadcast(msg);
  Serial.println("to fall1");
  //
