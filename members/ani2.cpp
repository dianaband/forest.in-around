...
void gotMessageCallback(uint32_t from, String & msg) { // REQUIRED
  ...
  if (receipent == IDENTITY) {
    ...
    {
    case ANI2_WORD_SING:
      Serial.println("voice2: ppi ppi ppi");
...
void greeting() {
  static String msg = "";
  sprintf(msg_cstr, "[%06d:%03d]", memberList[random(NUM_OF_MEMBERS)], ANI2_WORD_HELLO); //"Duncan. Living in the triangularities."
  msg = String(msg_cstr);
...
void routine() {
  static String msg = "";
  sprintf(msg_cstr, "[%06d:%03d]", ID_ANI1, ANI1_WORD_SING);
  msg = String(msg_cstr);
  mesh.sendBroadcast(msg);
  Serial.println("to ani1");
  //
